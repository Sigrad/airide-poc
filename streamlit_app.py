...
def load_data_pipeline():
    ...
    # Vereinheitlichung der Zeitspalte: Wir erzwingen 'datetime'
    if 'timestamp' in df_raw.columns:
        df_raw['datetime'] = pd.to_datetime(df_raw['timestamp'])
    elif 'datetime' in df_raw.columns:
        df_raw['datetime'] = pd.to_datetime(df_raw['datetime'])
    else:
        st.error("Fehler: Keine Zeitspalte (timestamp/datetime) gefunden.")
        return pd.DataFrame(), pd.DataFrame()
...
if df_raw.empty:
    st.error("Keine Daten geladen. Bitte 'real_waiting_times.csv' prüfen.")
else:
    # --- KPI BERECHNUNG ---
    latest_ts = df_raw['datetime'].max()
    snapshot = df_raw[df_raw['datetime'] == latest_ts]
    
    # Filter für offene Attraktionen
    ...
    c4.metric("HCI Index", f"{hci_score:.0f}/100")
    ...
    with tab1:
        ...
        with c_chart:
            ...
            if not open_rides.empty:
                ...
                st.info("Aktuell keine offenen Attraktionen.")
        ...
        with c_table:
            ...
            def style_status(val): return f"color: {'#ff4b4b' if val == 'Geschlossen' else '#09ab3b'}"
            
            st.dataframe(
                overview_df[['ride_name', 'Status', 'wait_time']].style.map(style_status, subset=['Status']),
                column_config={"wait_time": st.column_config.ProgressColumn("Wartezeit", format="%d min", min_value=0, max_value=120)},
    ...
    with tab2:
        ...
        if 'benchmark' in st.session_state:
            st.subheader("Bedeutung der Faktoren")
            rf_model = st.session_state['trainer'].models.get('rf')
            if rf_model:
                feat_cols = st.session_state['trainer'].feature_columns
                imp_df = pd.DataFrame({'Merkmal': feat_cols, 'Wert': rf_model.feature_importances_}).sort_values('Wert', ascending=False).head(10)
            ...
            st.info("Modelle müssen in der Seitenleiste trainiert werden.")
    
    # TAB 3 & 4 analog stabilisieren...