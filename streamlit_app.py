            ...
            with subtab_live:
                ...
                if df_today.empty: st.error("Keine Daten verfügbar.")
                    ...
                    if rows_live:
                        ...
                        with col_metrics:
                            st.markdown("**Metriken**"); mae = abs(live_df['Ist'] - live_df['Prognose_Schnitt']).mean(); st.metric("MAE", f"{mae:.2f} min"); st.markdown("**Modell-Abweichungen**"); live_df['Absoluter Fehler'] = abs(live_df['Ist'] - live_df['Random Forest']); st.dataframe(live_df[['Ist', 'Random Forest', 'Absoluter Fehler']].sort_values('Absoluter Fehler', ascending=False).head(5), use_container_width=True)
